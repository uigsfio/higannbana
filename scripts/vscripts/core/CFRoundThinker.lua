...
local ROUND_REST_TIME_BASE = 120
local ROUND_REST_WORLD = 0
-------------------------------------------------------------------------------------------
...
function CFRoundThinker:ThinkFighting()
	...
	-- 判断这一轮的怪物是否刷完了
	if CFSpawner:IsFinishedSpawn() then
		-- 判断结束条件
		if self._nCurrRound >= 3 then

			-- 如果BOSS被干掉了
			if CFSpawner:IsKilledBoss() then

				-- 让主程序addon_game_mode.lua结束游戏
				CHigannbanaGameMode:FinishedGame()
			end
		else
			-- 如果还有下一轮，那么
...
function CFRoundThinker:ReadAllEnemiesFromKv()
	local kv = LoadKeyValues( "scripts/maps/higannbana.txt" )
...
function CFRoundThinker:StartNextRound()
	...
	local randomround = self._nCurrRound * 2
	print( randomround )
	-- 读出这一轮怪的信息
	local tRoundData = self:ReadRoundData(RandomInt(randomround - 1, randomround ) )
