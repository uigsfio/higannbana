...
function CFSpawner:OnEntityKilled(keys)
	...
	print(killedUnit)
	-- 如果是BOSS，设置结束条件为真
	if killedUnit == "npc_hg_boss_invoker" then
		self._bBosskilled = true
	end
...
-------------------------------------------------------------------------------------------
-- 返回BOSS是否被击杀
function CFSpawner:IsKilledBoss()
	return self._bBosskilled
end
-------------------------------------------------------------------------------------------
-- 返回是否所有怪已经都被杀死
